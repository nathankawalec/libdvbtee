...
ssize_t socket_send(int sockfd, const void *buf, size_t len, int flags,
		    /*const struct sockaddr*/ void *dest_addr, socklen_t addrlen)
{
	if (sockfd < 0)
		return sockfd;
	int ret = 0;
...
#if 0
		((dest_addr) && (addrlen)) ?
			sendto(sockfd, buf, len, flags, (const struct sockaddr *) dest_addr, addrlen) :
			send(sockfd, buf, len, flags) :
#else
		sendto(sockfd, (const char *)buf, len, flags|MSG_NOSIGNAL, (const struct sockaddr *) dest_addr, addrlen) :
#endif
		ret;
}
